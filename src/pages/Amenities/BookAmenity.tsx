...
import { ChevronLeft, CalendarDays, Users, CreditCard, ChevronDown, ChevronUp } from 'lucide-react';
...
const BookAmenity: React.FC = () => {
  ...
  const calculateTotal = () => {
    const memberTotal = members.member.adult + members.member.child;
    const guestTotal = members.guest.adult + members.guest.child;
    const tenantTotal = members.tenant.adult + members.tenant.child;
    return memberTotal + guestTotal + tenantTotal;
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      postData.append('amenity_booking[member_child]', String(members.member.child));
      postData.append('amenity_booking[guest_adult]', String(members.guest.adult));
      postData.append('amenity_booking[guest_child]', String(members.guest.child));
      postData.append('amenity_booking[tenant_adult]', String(members.tenant.adult));
      postData.append('amenity_booking[tenant_child]', String(members.tenant.child));
  ...
  return (
    <div className="p-6 max-w-4xl mx-auto">
      ...
      <div className="flex items-center gap-2 text-sm text-muted-foreground mb-6">
        <button onClick={() => navigate('/amenities')} className="flex items-center gap-1 hover:text-foreground">
      ...
      <div className="bg-card border border-border rounded-xl p-6">
        ...
        <div className="border border-border rounded-lg">
          ...
          {termsOpen && (
            <div className="px-4 pb-4 text-sm text-muted-foreground">
              <p>By booking this facility, you agree to abide by all rules and regulations set forth by the management. The facility must be left in the same condition as it was found. Any damages will be charged to the booking party.</p>
            </div>
          )}
        </div>
      </div>
