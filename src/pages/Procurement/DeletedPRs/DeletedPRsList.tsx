...
const DeletedPRsList: React.FC = () => {
  ...
  const [data, setData] = useState<DeletedPR[]>([]);
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      // const response = await getDeletedPRsList(pagination.page, pagination.perPage);
      // setData(response.data);
      // setPagination(prev => ({ ...prev, total: response.total, totalPages: response.total_pages }));
      
      // Static placeholder data
      const mockData: DeletedPR[] = [];
      setData(mockData);
      setPagination(prev => ({ ...prev, total: 0, totalPages: 1 }));
    } catch (error) {
      console.error('Error fetching Deleted PRs:', error);
      setData([]);
    } finally {
      setLoading(false);
    }
  ...
  const handleView = (row: DeletedPR) => {
  ...
  // Table columns as per image - only 4 columns
  const columns: TableColumn<DeletedPR>[] = [
    {
      key: 'type',
      header: 'Type',
      sortable: true,
  ...
  return (
    <div className="space-y-4">
      {/* Title */}
      <h1 className="text-2xl font-bold text-foreground">Deleted PRs</h1>
      ...
      <div className="bg-card border border-border rounded-lg overflow-hidden">
        ...
        {data.length === 0 && (
          <div className="flex flex-col items-center justify-center py-12 border-t border-border">
            ...
            <p className="text-sm text-muted-foreground">No deleted PRs found</p>
          </div>
        )}
      </div>
