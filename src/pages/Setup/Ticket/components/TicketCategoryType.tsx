...
const TicketCategoryType: React.FC = () => {
  ...
  // const siteId = getItemInLocalStorage("SITEID");
  const siteId = String(getItemInLocalStorage("SITEID") || "");
  ...
  // Filter sub-categories based on search and selected category
  const filteredSubCategories = useMemo(() => {
    ...
    if (selectedCategoryFilter) {
      filtered = filtered.filter(
        (subCat) => String(subCat.helpdesk_category_name) === selectedCategoryFilter
      );
    }
    ...
    return filtered;
  }, [subCategories, selectedCategoryFilter, subCategorySearch]);
  ...
  const uniqueCategoryNames = useMemo(() => {
    return Array.from(
      new Set(
        subCategories
          .map((sc) => sc.helpdesk_category_name)
  ...
  return (
    <div className="space-y-6">
      {/* Add spacing above TabNavigation */}
      <div className="pt-4">
        <TabNavigation
          tabs={[
            { id: "category", label: "Category" },
      ...
      {activeTab === "category" && (
        <div className="space-y-4">
          {/* Search Bar and Add Category Button - Inline */}
          <div className="flex items-end gap-4">
            <div className="flex-1">
              ...
              <input
                ...
                className="w-full px-4 py-2.5 border rounded-lg bg-background text-foreground border-border focus:outline-none focus:ring-2 focus:ring-primary"
              />
            </div>
            <button
              className="px-4 py-2 text-sm bg-primary text-primary-foreground rounded-lg whitespace-nowrap"
              onClick={() => {
                setShowCategoryForm((prev) => !prev);
                setIsEditingCategory(false);
                setEditingCategoryId(null);
          ...
          <DataTable columns={categoryColumns} data={filteredCategories} />
      ...
      {activeTab === "sub-category" && (
        <div className="space-y-4">
          {/* Search Bar and Add Sub Category Button - Inline */}
          <div className="flex items-end gap-4">
          ...
          <DataTable columns={subCategoryColumns} data={filteredSubCategories} />
          
          {filteredSubCategories.length === 0 && (subCategorySearch || selectedCategoryFilter) && (
            <div className="text-center py-8 text-muted-foreground">
