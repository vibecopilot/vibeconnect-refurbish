...
//       id: 'service-desk',
//       label: 'SERVICE DESK',
//       defaultPath: '/setup/service-desk/ticket',
//       modules: [
//         { id: 'ticket', name: 'Ticket', path: '/setup/service-desk/ticket', feature: 'tickets', hidden: true,},
//       ],
//     },
//     {
//       id: 'crm',
...
//       {currentCategory?.modules &&
//   currentCategory.id !== 'usertree' &&
//   currentCategory.modules.filter(m => !m.hidden).length > 0 && (
//     <div className="border-b border-border bg-card">
//       <div className="w-full overflow-x-auto">
//         <div className="flex w-full min-w-max items-stretch px-6">
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="bg-white border-b border-gray-200 sticky top-0 z-40">
        ...
        <div className="relative">
          <div
            className={`flex items-center justify-between gap-2 pr-12 border-b border-gray-200 overflow-x-auto transition-all duration-300 ease-in-out ${
              isLevel1Visible ? 'max-h-16 py-2 opacity-100' : 'max-h-0 py-0 opacity-0 border-0'
            }`}
          >
            <div className="flex w-full min-w-max ">
              {visibleCategories.map((category) => (
          ...
          <button
            ...
            title={isLevel1Visible ? "Collapse categories" : "Expand categories"}
          >
            {isLevel1Visible ? (
      ...
      {currentCategory?.modules &&
        ...
        currentCategory.modules.filter(m => !m.hidden).length > 0 && (
          <div className="relative">
            <div
              className={`flex items-center justify-between gap-2 px-6 pr-12 border-b border-border overflow-x-auto bg-secondary/30 transition-all duration-300 ease-in-out ${
                isLevel2Visible ? 'max-h-14 py-2 opacity-100' : 'max-h-0 py-0 opacity-0 border-0'
              }`}
            >
              <div className="flex w-full min-w-max">
                {currentCategory.modules
            ...
            <button
              ...
              className="absolute right-3 top-1/2 -translate-y-1/2 w-7 h-7 flex items-center justify-center rounded-full border border-border bg-card hover:bg-accent hover:border-primary/50 shadow-sm transition-all duration-200 z-10 group"
              title={isLevel2Visible ? "Collapse modules" : "Expand modules"}
            >
              {isLevel2Visible ? (
                <ChevronUp className="w-4 h-4 text-muted-foreground group-hover:text-primary transition-colors" />
              ) : (
      ...
      {currentModule?.subTabs && currentModule.subTabs.length > 0 && (
        <div className="border-b border-border bg-card">
          <div className="flex w-full min-w-max px-6 -mx-6">
            {currentModule.subTabs.map((subTab) => (
              <button
                key={subTab.id}
                onClick={() => handleSubTabChange(subTab.path)}
