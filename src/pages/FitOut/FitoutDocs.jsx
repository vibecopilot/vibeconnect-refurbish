...
import {
  ...
  postFitoutDocsUpload,
  postFitOutSubCategoriesSetup,
  postHelpDeskSubCategoriesSetup,
} from "../../api";
const FitoutDocs = ({ handleToggleCategoryPage1, setCAtAdded }) => {
  ...
  useEffect(() => {
    const fetchCategory = async () => {
      try {
        const catResp = await getFitoutRequest();
        setCategories(catResp?.data?.fitout_requests);
      ...
      <div className="grid md:grid-cols-3 gap-6 ">
        ...
        <div className="flex flex-col gap-2">
          ...
          <input
            ...
            onChange={(e) => handleFileChange(e.target.files, "fitout_docs")}
            multiple={true}
          />
        </div>
        {/* <div className="flex flex-col gap-2">
          <select
            className="border p-2 rounded-md"
            value={formData.fitout_request_id}
