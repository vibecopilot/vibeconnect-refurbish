...
import { Loader2, Building2, AlertCircle, RefreshCw, Eye } from 'lucide-react';
...
const SpaceBookingsList: React.FC = () => {
  ...
  const fetchBookings = useCallback(async () => {
    ...
    try {
      ...
      // Filter for workspace/space type facilities
      const spaceTypes = ['workspace', 'space', 'meeting room', 'conference room', 'desk', 'seat', 'office'];
      
      // Combine booking data with facility details
      const combinedData = bookingsData.map((booking: SpaceBooking) => {
      ...
      // Filter for space-related bookings or show all if no specific type filter
      const spaceBookings = combinedData.filter((booking: SpaceBooking) => {
        const facType = booking.fac_type?.toLowerCase() || '';
        return spaceTypes.some(type => facType.includes(type)) || true; // Show all for now
      });

      const sortedData = spaceBookings.sort((a: SpaceBooking, b: SpaceBooking) => b.id - a.id);
  ...
  const handleExport = () => {
    ...
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Space Bookings');
      XLSX.writeFile(wb, 'Space_Bookings.xlsx');
      toast.dismiss();
  ...
  const columns: TableColumn<SpaceBooking>[] = [
    {
      ...
      width: '80px',
      render: (_, row) => (
        <Link to={`/space-booking/${row.id}`} className="text-muted-foreground hover:text-primary">
    ...
    return (
      <div className="p-6">
        ...
        <div className="flex flex-col items-center justify-center py-20">
          ...
          <p className="text-muted-foreground">Loading space bookings...</p>
        </div>
      </div>
    );
  }
...
export default SpaceBookingsList;
