...
// //         ? vendorData
// //         : vendorData?.vendors || vendorData?.data || []
// //     );
// //   } catch (error) {
...
// //     submitData.append(
// //       `other_bill[${key}]`,
// //       key === 'vendor_id' ? Number(value).toString() : value
// //     );
// //   }
// // });
...
//         const ded = parseFloat(name === 'deduction' ? value : updated.deduction) || 0;
//         updated.amount = String(base - ded);
//       }

//       // Get the current amount for calculations
//       const amt = parseFloat(updated.amount) || 0;

//       // Calculate TDS amount
//       if (name === 'tds_rate' || name === 'amount' || name === 'base_amount' || name === 'deduction') {
//         const rate = parseFloat(updated.tds_rate) || 0;
//         updated.tds_amount = String((amt * rate / 100).toFixed(2));
//       }

//       // Calculate CGST amount
//       if (name === 'cgst_rate' || name === 'amount' || name === 'base_amount' || name === 'deduction') {
//         const rate = parseFloat(updated.cgst_rate) || 0;
//         updated.cgst_amount = String((amt * rate / 100).toFixed(2));
//       }

//       // Calculate SGST amount
//       if (name === 'sgst_rate' || name === 'amount' || name === 'base_amount' || name === 'deduction') {
//         const rate = parseFloat(updated.sgst_rate) || 0;
//         updated.sgst_amount = String((amt * rate / 100).toFixed(2));
//       }

//       // Calculate IGST amount
//       if (name === 'igst_rate' || name === 'amount' || name === 'base_amount' || name === 'deduction') {
//         const rate = parseFloat(updated.igst_rate) || 0;
//         updated.igst_amount = String((amt * rate / 100).toFixed(2));
//       }

//       // Calculate TCS amount
//       if (name === 'tcs_rate' || name === 'amount' || name === 'base_amount' || name === 'deduction') {
//         const rate = parseFloat(updated.tcs_rate) || 0;
//         updated.tcs_amount = String((amt * rate / 100).toFixed(2));
//       }

//       // Calculate Total Amount
//       // Total = Amount + CGST + SGST + IGST + TCS + Additional Expenses - TDS
...
//       const tcs = parseFloat(updated.tcs_amount) || 0;
//       const tds = parseFloat(updated.tds_amount) || 0;
//       const additionalExp = parseFloat(updated.additional_expenses) || 0;
      
//       updated.total_amount = String((amt + cgst + sgst + igst + tcs + additionalExp - tds).toFixed(2));
...
const CreateOtherBill = () => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement | HTMLSelectElement>) => {
    ...
    setFormData((prev) => {
      ...
      // Calculate Tax Amount (sum of all taxes)
      // Tax Amount = CGST + SGST + IGST + TCS
      const cgst = parseFloat(updated.cgst_amount) || 0;
      const sgst = parseFloat(updated.sgst_amount) || 0;
      const igst = parseFloat(updated.igst_amount) || 0;
      const tcs = parseFloat(updated.tcs_amount) || 0;
      
      updated.tax_amount = String((cgst + sgst + igst + tcs).toFixed(2));
