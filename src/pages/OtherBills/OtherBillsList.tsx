...
// import { getOtherBills, getVendors } from '../../api';
...
//     const vendorArray = Array.isArray(res.data)
//       ? res.data
//       : res.data?.vendors || [];
...
// //     acc[v.id] = v.company_name || v.vendor_name;
// //     return acc;
...
//           bill.suplier_name?.toLowerCase().includes(searchValue.toLowerCase()) ||
...
//                 <p><span className="font-medium">Supplier:</span> {bill.suplier_name || bill.vendor_name || '-'}</p>
...
//                     <td className="px-4 py-3 text-sm text-foreground whitespace-nowrap">{bill.suplier_name || bill.vendor_name || '-'}</td>
...
import { useState, useEffect } from 'react';
import { useNavigate, Link } from 'react-router-dom';
import { Eye, Edit, DollarSign, FileText, CheckCircle, Clock, ArrowUpDown, ArrowUp, ArrowDown } from 'lucide-react';
import Breadcrumb from '../../components/layout/Breadcrumb';
import ListToolbar from '../../components/ui/ListToolbar';
import { getOtherBills, getVendors } from '../../api';
import { dateFormatSTD } from '../../utils/dateUtils';
...
interface OtherBill {
  ...
  suplier_name?: string;
  vendor_name?: string;
  last_approved_by?: string | { firstname?: string; lastname?: string };
  total_amount?: number;
  deduction?: number;
  tds_percentage?: number;
  tds_amount?: number;
  ...
  payment_status?: string;
  created_at?: string;
  created_by_name?: string | { firstname?: string; lastname?: string };
}
...
type SortField = keyof OtherBill;
...
const OtherBillsList = () => {
  ...
  const getUserDisplayName = (user?: string | { firstname?: string; lastname?: string }) => {
    if (!user) return '-';
  ...
  useEffect(() => {
    filterBills();
  }, [searchValue, dateFilter, bills, sortField, sortDirection]);
  ...
  const filterBills = () => {
    ...
    if (sortField && sortDirection) {
      filtered.sort((a, b) => {
        ...
        // Convert user objects to strings for comparison
        if (sortField === 'last_approved_by' || sortField === 'created_by_name') {
          aValue = getUserDisplayName(aValue as any);
          bValue = getUserDisplayName(bValue as any);
        }
        ...
        <div className="bg-card border border-border rounded-xl overflow-hidden">
          <div className="overflow-x-auto">
            <table className="w-full min-w-[2000px]">
              <thead className="bg-muted">
                <tr>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground uppercase whitespace-nowrap">Action</th>
                  <th 
                    className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground uppercase whitespace-nowrap cursor-pointer hover:bg-muted/80"
                  ...
                  <th 
                    className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground uppercase whitespace-nowrap cursor-pointer hover:bg-muted/80"
                    onClick={() => handleSort('suplier_name')}
                  >
                    Supplier {getSortIcon('suplier_name')}
                  </th>
                  <th 
                    className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground uppercase whitespace-nowrap cursor-pointer hover:bg-muted/80"
                    onClick={() => handleSort('last_approved_by')}
                  >
                    Last Approved By {getSortIcon('last_approved_by')}
                  </th>
                  <th 
                    className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground uppercase whitespace-nowrap cursor-pointer hover:bg-muted/80"
                  ...
                  <th 
                    className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground uppercase whitespace-nowrap cursor-pointer hover:bg-muted/80"
                    onClick={() => handleSort('tds_percentage')}
                  >
                    TDS(%) {getSortIcon('tds_percentage')}
                  </th>
                  <th 
                    className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground uppercase whitespace-nowrap cursor-pointer hover:bg-muted/80"
                    onClick={() => handleSort('tds_amount')}
                  >
                    TDS Amount {getSortIcon('tds_amount')}
                  </th>
                  <th 
                    className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground uppercase whitespace-nowrap cursor-pointer hover:bg-muted/80"
                    onClick={() => handleSort('retention_percentage')}
                  >
                    Retention(%) {getSortIcon('retention_percentage')}
                  </th>
                  <th 
                    className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground uppercase whitespace-nowrap cursor-pointer hover:bg-muted/80"
                    onClick={() => handleSort('retention_amount')}
                  >
                    Retention Amount {getSortIcon('retention_amount')}
                  </th>
                  <th 
                    className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground uppercase whitespace-nowrap cursor-pointer hover:bg-muted/80"
                    onClick={() => handleSort('payable_amount')}
                  >
                    Payable Amount {getSortIcon('payable_amount')}
                  </th>
                  <th 
                    className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground uppercase whitespace-nowrap cursor-pointer hover:bg-muted/80"
                    onClick={() => handleSort('bill_date')}
                  >
                    Bill Date {getSortIcon('bill_date')}
                  </th>
                  <th 
                    className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground uppercase whitespace-nowrap cursor-pointer hover:bg-muted/80"
                  ...
                  >
                    PAN Number {getSortIcon('pan_number')}
                  </th>
                  <th 
                    className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground uppercase whitespace-nowrap cursor-pointer hover:bg-muted/80"
                    onClick={() => handleSort('payment_tenure')}
                  >
                    Payment Tenure {getSortIcon('payment_tenure')}
                  </th>
                  <th 
                    className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground uppercase whitespace-nowrap cursor-pointer hover:bg-muted/80"
                  ...
                  >
                    Created On {getSortIcon('created_at')}
                  </th>
                  <th 
                    className="px-4 py-3 text-left text-xs font-semibold text-muted-foreground uppercase whitespace-nowrap cursor-pointer hover:bg-muted/80"
              ...
              <tbody className="divide-y divide-border">
                {paginatedData.map((bill) => (
                  <tr key={bill.id} className="hover:bg-muted/50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-sm text-foreground max-w-[200px] truncate">{bill.description || '-'}</td>
                    <td className="px-4 py-3 text-sm text-foreground whitespace-nowrap">{bill.suplier_name || bill.vendor_name || '-'}</td>
                    <td className="px-4 py-3 text-sm text-foreground whitespace-nowrap">{getUserDisplayName(bill.last_approved_by)}</td>
                    <td className="px-4 py-3 text-sm text-foreground whitespace-nowrap">{formatCurrency(bill.total_amount)}</td>
                    <td className="px-4 py-3 text-sm text-foreground whitespace-nowrap">{formatCurrency(bill.deduction)}</td>
                    <td className="px-4 py-3 text-sm text-foreground whitespace-nowrap">{bill.tds_percentage ?? '-'}%</td>
                    <td className="px-4 py-3 text-sm text-foreground whitespace-nowrap">{formatCurrency(bill.tds_amount)}</td>
                    ...
                    <td className="px-4 py-3 text-sm text-foreground whitespace-nowrap">{bill.created_at ? dateFormatSTD(bill.created_at) : '-'}</td>
                    <td className="px-4 py-3 text-sm text-foreground whitespace-nowrap">{getUserDisplayName(bill.created_by_name)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
