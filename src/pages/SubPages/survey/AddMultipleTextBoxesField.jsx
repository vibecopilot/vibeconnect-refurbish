...
function AddMultipleTextBoxesField() {
    ...
    <div className="flex flex-col w-full overflow-hidden">
      <div className="grid grid-cols-2 gap-5 border-t">
        <div className="w-full mx-auto col-span-2">
          ...
          <div className="col-span-2 flex gap-5 items-center justify-start border-t border-b py-3 px-4">
            <div>
              <input
                type="checkbox"
                id=""
            ...
            {isNumerical && (
              <div>
                <input
                  ...
                  id=""
                  checked={isFixedSum}
                  onChange={() => setIsFixedSum(!isFixedSum)}
                  className="mr-2 w-3 h-3"
                />
                <label htmlFor="" className="text-sm font-medium text-gray-600">
                  Require a Fixed Sum
          ...
          {isNumerical && (
            <div className="w-full my-5">
              <h2 className="text-sm text-gray-800">
                Customize Number Validation Message
              </h2>
              <div className="px-5 py-2">
                <label className="text-sm text-gray-800">
                  When an answer is not a number, display this error message.
                </label>
                <textarea
                  placeholder="Please enter a number. Decimals, percentages, and non-numeric characters are not accepted."
          ...
          {isFixedSum && (
            <div className="w-full my-5 border-t pt-5">
              <h2 className="text-sm text-gray-800">Require a Fixed Sum</h2>
              <div className="px-5 py-2 flex gap-5 items-center w-full">
                <label className="text-sm text-gray-800">
                  Sum of all Answers =
                </label>
                <input
                  type="text"
                  placeholder=""
                  className="border px-2 py-2 rounded-md w-32 min-w-[8rem]"
                />
              </div>
              <div className="px-5 py-2">
                <label className="text-sm text-gray-800">
                  When the answers do not add up correctly, display this error
                  message.
                </label>
                <textarea
                  placeholder="The choices need to add up to [enter sum here]."
                  className="border px-2 py-2 rounded-md w-full resize-none"
                  rows="3"
                />
              </div>
            </div>
...
export default AddMultipleTextBoxesField;
