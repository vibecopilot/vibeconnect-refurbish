...
const operationalSubTabs = [
  ...
  { id: "conducted", label: "Conducted", path: "/audit/operational/conducted" },
  {
    id: "checklists",
    label: "Checklists",
    path: "/audit/operational/checklists",
  },
];
...
const AuditLayout: React.FC = () => {
  ...
  const currentMainTab = mainTabs.find((tab) =>
    location.pathname.startsWith(tab.path)
  );
  const subTabs = isOperational
    ? operationalSubTabs
    : isVendor
    ? vendorSubTabs
    : [];
  const currentSubTab = subTabs.find((tab) =>
    location.pathname.startsWith(tab.path)
  );
  ...
  return (
    <div className="p-6">
      ...
      {/* Main Tab Navigation (Operational / Vendor) - ALWAYS VISIBLE */}
      <div className="mb-4">
        <nav className="flex gap-2 border-b border-border">
          {mainTabs.map((tab) => (
            <button
              key={tab.id}
              onClick={() => navigate(tab.path + "/scheduled")}
              className={`px-4 py-2.5 text-sm whitespace-nowrap transition-colors relative uppercase
              ${
                currentMainTab?.id === tab.id
