...
const VariableDeduction = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      selector: (row) => row.percentage_of_salary,
      sortable: true,
    },
    {
      name: "Value",
  ...
  const handleDeleteFixedDeduction = async (deductionId) => {
    try {
      await deleteHrmsFixedDeduction(deductionId);
      toast.success("Variable Deduction deleted successfully");
  ...
  const handleAddVariableDeduction = async () => {
    ...
    try {
      ...
      postData.append("deduction_type", formData.deduction_type);
      postData.append("percentage_of_salary", formData.percentage_of_salary);
      postData.append("value", formData.value);
      postData.append("organization", hrmsOrgId);

      await postHrmsFixedDeduction(postData);
      toast.success("Variable Deduction added successfully");
      closeModal();
      fetchVariableDeductions();
    } catch (error) {
      console.log(error);
      toast.error("Failed to add Variable Deduction");
    }
  };
          ...
          <div className="max-h-[100%] bg-white p-8 w-2/3 rounded-lg shadow-lg ">
            ...
            <div>
              <div className="grid md:grid-cols-2 gap-5 my-5 max-h-96 overflow-y-auto p-1">
                ...
                <div className="grid gap-2 items-center w-full">
                  <label className="block mb-1 font-semibold">
                    Percentage of Salary <span className="text-red-500">*</span>
                  </label>
                  <input
                    type="number"
              ...
              <div className="flex mt-2 justify-end gap-2 p-1 border-t">
                ...
                <button
                  type="button"
                  onClick={handleAddVariableDeduction}
        ...
        <div className="flex flex-col mt-4 mr-1 bg-gray-50 rounded-md text-wrap  gap-4 my-2 py-2 pl-5 pr-2 w-[18rem]">
          ...
          <div className=" ">
            <ul style={listItemStyle} className="flex flex-col gap-2">
              ...
              <li>
                <p>
                  You can change deduction setting anytime but once payroll is
