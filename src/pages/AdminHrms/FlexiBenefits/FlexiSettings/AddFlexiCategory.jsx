...
import { postFlexiBenefitCategory } from "../../../../api";
import toast from "react-hot-toast";

const AddFlexiCategory = ({ setPage, fetchFlexiCategory }) => {
  ...
  const handleAddFlexiBenefitCategory = async () => {
    const postData = new FormData();
    postData.append("benefit_type", formData.benefitType);
    postData.append("label", formData.customLabel);
    postData.append("frequency", formData.frequency);
    // postData.append("icon", null);
    postData.append("unclaimed_amount_action", formData.unclaimedAction);
    postData.append(
    ...
    postData.append("fnf_unclaimed_action", formData.unclaimedFNF);
    postData.append("unclaimed_amount_label", formData.unclaimedLabel);
    postData.append("pf_wage", formData.unclaimedPartOfPFWage);
    postData.append("esic_wage", formData.unclaimedPartOfESICWage);
    postData.append("pt_wage", formData.unclaimedPartOfPTWage);
    postData.append("lwf_wage", formData.unclaimedPartOfLwfWage);
    postData.append(
      "max_submission_times",
      formData.timesEmployeeSubmitFlexiBenefit
    );
    postData.append("pro_rata_eligibility", formData.proRateEligibility);
    postData.append("surplus_amount_action", formData.surplusAmountAction);
    postData.append("taxed_from_month", formData.taxedMonth);
    postData.append("organization", hrmsOrgId);
    try {
      const res = await postFlexiBenefitCategory(postData);
      toast.success("Flexi Benefit Created successfully");
      fetchFlexiCategory();
      setPage();
    } catch (error) {
      console.log(error);
    ...
    <div className="mb-5">
      ...
      <div className="flex gap-4 justify-center items-center my-4">
        ...
        <button
          ...
          onClick={handleAddFlexiBenefitCategory}
        >
...
export default AddFlexiCategory;
